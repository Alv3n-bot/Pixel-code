  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-purple-600/20 blur-[120px] rounded-full will-change-transform"></div>
        ...
        <div className="relative z-10 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 md:gap-6 lg:gap-8 max-w-7xl mx-auto">
          {SERVICES.map((service, idx) => (
            <div
              key={service.id}
              className="group relative backdrop-blur-md bg-slate-900/50 border border-purple-500/20 rounded-xl md:rounded-2xl lg:rounded-3xl p-3 sm:p-4 md:p-6 lg:p-8 hover:border-purple-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-purple-500/30 transform-gpu will-change-transform"
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="relative w-full max-w-md bg-slate-900 border border-purple-500/30 rounded-2xl p-6 md:p-8 shadow-xl shadow-purple-500/20 transform-gpu">
            <button
              onClick={closeModal}
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  <label className="flex items-center gap-2 text-slate-300 text-sm cursor-pointer">
                    ...
                    />
                    <span className="leading-tight">Pay 50% Deposit ({formatPrice(selectedService?.price * 0.5)})</span>
                  </label>
                  <label className="flex items-center gap-2 text-slate-300 text-sm cursor-pointer">
                    <input
                      type="radio"
                      ...
                      className="text-purple-500 focus:ring-purple-500"
                    />
                    <span className="leading-tight">Pay 100% Full ({formatPrice(selectedService?.price)})</span>
                  </label>
                </div>
              </div>
              <button
                type="submit"
                className="w-full mt-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-lg transition-all duration-300 shadow-lg shadow-purple-500/30 transform-gpu"
              >
                Proceed to Payment
              </button>
            </form>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="relative w-full max-w-md bg-slate-900 border border-purple-500/30 rounded-2xl p-6 md:p-8 shadow-xl shadow-purple-500/20 text-center transform-gpu">
            <button
            ...
            <p className="text-purple-300 mb-4 font-mono bg-slate-800/50 px-3 py-1 rounded-lg text-sm break-all">
              Reference: {successData?.reference}
            </p>
            <p className="text-sm text-slate-300 mb-6">
              We'll contact you at <span className="font-semibold text-white break-all">{successData?.email}</span> to start your project.
            </p>
            <button
              onClick={closeSuccess}
